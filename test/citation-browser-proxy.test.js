...
function fixture({env={},supplier,runError,launchError,eventError,receiptError}={}){
 ...
 const browser={async newContext(){return {route:async()=>{},close:async()=>{calls.contextClose++;},newPage:async()=>({setDefaultTimeout(){},evaluate:async()=>({found:true,groups:0,unresolved:false,sources:[]}),evaluateHandle:async()=>({evaluate:async fn=>fn({isCurrent:()=>true}),dispose:async()=>{}}),screenshot:async()=>Buffer.from('fixture')})};},async close(){calls.browserClose++;if(runtime.browserCleanup)await runtime.browserCleanup();}};
