...
function shouldClickSourceExpansionButton(text, aria) {
...
async function expandAIOverviewSources(page) {
  const matches = await page.evaluate(
    ({
      ...
      return Array.from(root.querySelectorAll(buttonSelector)).map((button, index) => {
        if (!(button instanceof HTMLElement)) {
          return null;
        }
        const style = window.getComputedStyle(button);
        const rect = button.getBoundingClientRect();
        const visible = button.isConnected && style.display !== "none" && style.visibility !== "hidden" && style.opacity !== "0" && rect.width > 0 && rect.height > 0;
  ...
  for (const match of matches) {
    ...
    const button = page.locator(`${AI_OVERVIEW_SOURCE_ROOT_SELECTOR} ${AI_OVERVIEW_BUTTON_SELECTOR}`).nth(match.index);
    const visible = await button.isVisible().catch(() => false);
    if (!visible) continue;
    logger.debug(
      `[ai-overview] expanding sources text="${match.text}" aria="${match.aria}"`
    );
    await button.click({ timeout: 5e3 }).catch(() => {
    });
    await page.waitForTimeout(700);
  }
}
export {
  expandAIOverviewSources
};
