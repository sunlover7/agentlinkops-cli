...
const PERPLEXITY_RAW_SOURCES_DOM_EXTRACTOR = String.raw`(_helpers) => {
	const results = [];
...
async function extractSourcesFromPerplexity(page, sourcesButton) {
  ...
  if (!clickedToClose) {
    const escaped2 = await pressKeyLikeUser(page, "Escape").catch(() => false);
    if (!escaped2 && canUseOsLevelInput(page)) {
...
export {
  PERPLEXITY_RAW_SOURCES_DOM_EXTRACTOR,
  extractSourcesFromPerplexity
};
