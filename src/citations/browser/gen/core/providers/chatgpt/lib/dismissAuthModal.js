...
async function dismissChatgptAuthModal(page, options = {}) {
  const waitForAppearanceMs = options.waitForAppearanceMs ?? 0;
  const deadline = Date.now() + waitForAppearanceMs;
  do {
    const dialog = page.locator(AUTH_DIALOG_SELECTOR).filter({ hasText: AUTH_DIALOG_TEXT_RE }).first();
    ...
    await dialog.waitFor({ state: "hidden", timeout: 5e3 }).catch(() => {
    });
