...
const XVFB_START_TIMEOUT_MS = 5e3;
...
async function waitForDisplaySocket(displayNumber, child, timeoutMs = XVFB_START_TIMEOUT_MS) {
...
async function ensureDisplay(options) {
  if ((process.env.CAMOUFOX_HEADLESS_MODE ?? DEFAULT_CAMOUFOX_HEADLESS_MODE) === "headless") {
    return null;
  }
  ...
  if ((process.env.CAMOUFOX_HEADLESS_MODE ?? DEFAULT_CAMOUFOX_HEADLESS_MODE) === "headful") {
    throw new Error(
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const display = preferredDisplay ?? `:${100 + Math.floor(Math.random() * 800)}`;
