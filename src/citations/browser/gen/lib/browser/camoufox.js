...
import { resolveAppMode } from "../../../shims/types.js";
const execFileAsync = promisify(execFile);
const PYTHON_CANDIDATES = ["python3.12", "python3.11", "python3.10", "python3"];
const PYTHON_PROBE_TIMEOUT_MS = 5e3;
const CAMOUFOX_OPTIONS_TIMEOUT_MS = 3e4;
const SYSTEM_FONTS_TIMEOUT_MS = 15e3;
...
let cachedSystemFontFamilies = null;
let pendingSystemFontFamilies = null;
...
function getHumanizeValue() {
  if (!CAMOUFOX_HUMANIZE) return false;
  const maxTime = CAMOUFOX_HUMANIZE_MAX_TIME_S;
  return maxTime > 0 ? maxTime : true;
}
function resolveHeadlessMode(override) {
  return override ?? process.env.CAMOUFOX_HEADLESS_MODE ?? DEFAULT_CAMOUFOX_HEADLESS_MODE;
}
function getHeadlessValue(override) {
  return resolveHeadlessMode(override) === "headless";
}
async function canUsePythonBinary(candidate) {
  try {
    ...
    return typeof parsed.major === "number" && typeof parsed.minor === "number" && (parsed.major > 3 || parsed.major === 3 && parsed.minor >= 10);
  } catch {
    return false;
  }
}
async function resolvePythonBinary(provider) {
...
function parseJsonValue(name, raw) {
...
function parseJsonRecord(name, raw) {
  const parsed = parseJsonValue(name, raw);
...
function parsePrimitiveRecord(name, raw) {
...
function toPrimitiveRecord(name, value) {
...
function parseStringList(name, raw) {
  if (!raw) return void 0;
  const trimmed = raw.trim();
  if (!trimmed) return void 0;
  if (trimmed.startsWith("[")) {
    const parsed = parseJsonValue(name, trimmed);
...
function parseStringOrList(name, raw) {
  const parsed = parseStringList(name, raw);
  if (!parsed || parsed.length === 0) return void 0;
  return parsed.length === 1 ? parsed[0] : parsed;
}
function parseStringPair(name, raw) {
  if (!raw) return void 0;
  const parsed = parseJsonValue(name, raw);
...
function parseFingerprintPreset(raw) {
  if (!raw) return void 0;
...
function parseFontFamilyLines(raw) {
  return dedupeStrings(
    raw.split(/\r?\n/).flatMap((line) => line.split(",")).map((part) => normalizeFontFamilyName(part)).filter((value) => Boolean(value))
  );
}
async function listFontsWithFcList() {
...
async function listFontsWithSystemProfiler() {
  const { stdout } = await execFileAsync(
    "system_profiler",
  ...
  const parsed = JSON.parse(stdout);
  return dedupeStrings(
    (parsed.SPFontsDataType ?? []).flatMap((entry) => entry.typefaces ?? []).map((typeface) => normalizeFontFamilyName(typeface.family ?? "")).filter((value) => Boolean(value))
  );
}
async function listFontsWithPowerShell() {
  const script = [
  ...
  return dedupeStrings(
    parsed.map(
      (value) => typeof value === "string" ? normalizeFontFamilyName(value) : null
...
async function buildLaunchPayload(args) {
  const extraLaunch = args.plainAuthMode ? {} : args.disableFingerprinting ? {} : parseJsonRecord(
    "CAMOUFOX_EXTRA_LAUNCH_JSON",
    process.env.CAMOUFOX_EXTRA_LAUNCH_JSON
  ) ?? {};
  const extraArgs = Array.isArray(extraLaunch.args) && extraLaunch.args.every((item) => typeof item === "string") ? extraLaunch.args : void 0;
  const payload = { ...extraLaunch };
  const config = {
    ...args.plainAuthMode ? {} : {
  ...
  const firefoxUserPrefs = {
    ...args.plainAuthMode ? {
      ...
      ...isJsonRecord(extraLaunch.firefox_user_prefs) ? extraLaunch.firefox_user_prefs : {},
      ...parseJsonRecord(
  ...
  payload.env = {
    ...args.plainAuthMode ? {} : toPrimitiveRecord("CAMOUFOX_EXTRA_LAUNCH_JSON.env", extraLaunch.env) ?? {},
    ...pickBrowserEnv({
  ...
  payload.humanize = isLocalAppMode() || headlessMode === "headful" ? false : typeof args.humanize === "boolean" ? args.humanize : getHumanizeValue();
  if (!args.plainAuthMode) {
    ...
    payload.custom_fonts_only = process.env.CAMOUFOX_CUSTOM_FONTS_ONLY === "true";
  ...
  payload.i_know_what_im_doing = process.env.CAMOUFOX_I_KNOW_WHAT_IM_DOING === "true";
  payload.debug = process.env.CAMOUFOX_DEBUG === "true";
  if (args.proxy) {
    payload.proxy = args.proxy;
  } else {
    payload.proxy = void 0;
  }
  ...
  const os = args.plainAuthMode ? resolveHostOs() : parseStringOrList("CAMOUFOX_OS", process.env.CAMOUFOX_OS) ?? (isLocalAppMode() && resolveHeadlessMode(args.headlessMode) === "headful" ? resolveHostOs() : void 0);
  if (os !== void 0) payload.os = os;
  const locale = args.plainAuthMode ? "en-US" : parseStringOrList("CAMOUFOX_LOCALE", process.env.CAMOUFOX_LOCALE) ?? "en-US";
  payload.locale = locale;
  const addons = args.plainAuthMode ? void 0 : parseStringList("CAMOUFOX_ADDONS", process.env.CAMOUFOX_ADDONS);
  if (addons !== void 0) payload.addons = addons;
  const fonts = args.plainAuthMode ? void 0 : parseStringList("CAMOUFOX_FONTS", process.env.CAMOUFOX_FONTS);
  const systemFonts = args.plainAuthMode ? await discoverSystemFontFamilies() : process.env.CAMOUFOX_USE_FULL_OS_FONTS === "true" ? await discoverSystemFontFamilies() : [];
  const mergedFonts = dedupeStrings([...fonts ?? [], ...systemFonts]);
  if (mergedFonts.length > 0) payload.fonts = mergedFonts;
  payload.use_full_os_fonts = args.plainAuthMode ? true : process.env.CAMOUFOX_USE_FULL_OS_FONTS === "true";
  if (args.plainAuthMode) {
  ...
  const excludeAddons = args.plainAuthMode ? void 0 : parseStringList(
    "CAMOUFOX_EXCLUDE_ADDONS",
    process.env.CAMOUFOX_EXCLUDE_ADDONS
  )?.map((value) => value.toUpperCase());
  if (excludeAddons !== void 0) payload.exclude_addons = excludeAddons;
  const webglConfig = args.plainAuthMode ? void 0 : parseStringPair(
    "CAMOUFOX_WEBGL_CONFIG",
    process.env.CAMOUFOX_WEBGL_CONFIG
  );
  if (webglConfig !== void 0) payload.webgl_config = webglConfig;
  const ffVersion = args.plainAuthMode ? void 0 : parsePositiveInteger(
    "CAMOUFOX_FF_VERSION",
    process.env.CAMOUFOX_FF_VERSION
  );
  if (ffVersion !== void 0) payload.ff_version = ffVersion;
  if (args.plainAuthMode || args.disableFingerprinting) {
    payload.fingerprint_preset = false;
    payload.fingerprint = void 0;
  } else {
    const fingerprint = parseJsonRecord(
      "CAMOUFOX_FINGERPRINT_JSON",
      process.env.CAMOUFOX_FINGERPRINT_JSON
    ) ?? (isJsonRecord(extraLaunch.fingerprint) ? extraLaunch.fingerprint : void 0);
    if (fingerprint !== void 0) payload.fingerprint = fingerprint;
    const fingerprintPreset = parseFingerprintPreset(
      process.env.CAMOUFOX_FINGERPRINT_PRESET
    );
    if (fingerprintPreset !== void 0) {
      payload.fingerprint_preset = fingerprintPreset;
    }
  }
  const argsList = args.plainAuthMode ? void 0 : parseStringList("CAMOUFOX_ARGS", process.env.CAMOUFOX_ARGS) ?? extraArgs;
  if (argsList !== void 0) payload.args = argsList;
  ...
  for (const key of Object.keys(payload)) {
    ...
    if (value === void 0 || isJsonRecord(value) && Object.keys(value).length === 0 || Array.isArray(value) && value.length === 0) {
      delete payload[key];
    }
  }
  return payload;
}
async function resolveCamoufoxLaunchOptions(args) {
  ...
  try {
    ...
    const installHint = "Install Camoufox in the runtime image, for example: python3 -m pip install 'cloverlabs-camoufox[geoip]' && python3 -m camoufox set official/stable && python3 -m camoufox fetch";
    throw new ExternalServiceError(
      args.provider,
      message.includes("CAMOUFOX_IMPORT_ERROR::") ? `Camoufox is not installed for ${python}. ${installHint}` : `Failed to resolve Camoufox launch options: ${message}`
    );
  }
}
export {
