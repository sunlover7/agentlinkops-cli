...
import { createEgressJournal } from './egress-receipts.js';
...
export async function runEpoch(panelInput, options = {}) {
  // Supplied adapters transfer epoch cleanup responsibility to this runner.
  const uniqueEngines = [...new Set(options.engines?.values() ?? [])];
  let journal, primaryError;
  try {
    ...
    const browserEngines = uniqueEngines.filter(engine => typeof engine.getEgressReceipts === 'function' || cells.some(cell => cell.engine === engine && cell.identity.includes(':web-own-browser')));
    journal = await createEgressJournal({ dir, epochId, engines: browserEngines, identityHashes: new Map(cells.map(cell => [cell.engine, sha256(cell.identity)])) });
    outer:
      ...
      for (let runIndex = 0; runIndex < samples; runIndex += 1) {
        ...
        for (let attempt = 0; attempt < 2 && !run; attempt += 1) {
          ...
          try {
            ...
            await journal?.capture(cell.engine, { phase: 'attempt', error: attemptError, cellHash: sha256(cell.id), runIndex, attempt });
          }
        }
        if (!run) {
    ...
    let persistenceError, cleanupError;
    for (const engine of uniqueEngines) {
      let error;
      try { await engine.close?.(); } catch (cause) { error = cause; cleanupError ??= cause; }
      try { await journal?.capture(engine, { phase: 'cleanup', error }); }
      catch (cause) { persistenceError ??= cause; }
    }
    try { await journal?.finish({ failed: Boolean(primaryError || persistenceError || cleanupError), cleanupUncertain: Boolean(cleanupError), evidenceComplete: !persistenceError && !String(primaryError?.code ?? '').startsWith('EGRESS_') }); }
    catch (cause) { persistenceError ??= cause; }
    try { await journal?.close(); } catch (cause) { persistenceError ??= cause; }
    if (persistenceError) throw persistenceError;
    if (!primaryError && cleanupError && journal) throw Object.assign(new EngineError('Browser cleanup could not be confirmed; inspect the egress journal.'), { code: 'BROWSER_CLEANUP_UNCONFIRMED' });
  }
}
